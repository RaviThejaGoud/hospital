...
import com.lowagie.text.pdf.PdfWriter;
 ...
 public class PdfHeaderFooterMarkJasper extends PdfPageEventHelper {
	...
	/**
	 ...
	 /**
	    ...
	    public void onEndPage(PdfWriter writer, Document document) {
	        ...
	        // compose the footer
	        String text = " Page " +  (writer.getCurrentPageNumber()) + " of ";
	        String dateStr=DateFormatter.formatDate(DateFormatter.MMMMDCCYY_PATTERN,new  Date());
	    	...
	    	//tpl.getPdfDocument().getPageSize().getWidth();
	    	cb.setColorStroke(Color.BLACK);
	    	float x = 85f;
	    	float y = 55f;
	    	cb.moveTo(20,y);
	    	cb.lineTo(x + 72f*10, y);
	        cb.stroke();
	        
	        //String disclaimer=""+"";
	        cb.setTextMatrix(document.left(), textBase);
            cb.showText("DISCLAIMER : The information contained in this document may be privileged, confidential, and protected from disclosure. You are hereby notified that any dissemination, distribution, or copying of this document outside of its intended use is prohibited");
            cb.endText();
            cb.saveState();
	     ...
	     // Create a Date to the left side of the footer 
            ...
            float adjust1 = helv.getWidthPoint(text, 12);
            cb.setTextMatrix(document.right() - textSize-adjust1+2, textBase);
            cb.showText(dateStr+" |");
            cb.endText();	  
          //  cb.addTemplate(tpl, document.right() - textSize-10, textBase);	  
            cb.saveState();  
         
	       // Create a page numbers to the right side of the footer
	            textSize = helv.getWidthPoint(text, 10);
		        textBase = document.bottom() - 20;
		        cb.beginText();
		        cb.setFontAndSize(helv, 8);
	            float adjust = helv.getWidthPoint(text, 12);
	            cb.setTextMatrix(document.right() - textSize-adjust+50+5, textBase);
	            cb.showText(text);
	            cb.endText();
	            cb.saveState();
	            cb.beginText();
	            cb.setFontAndSize(helv, 7);
	            cb.addTemplate(tpl, document.right() - textSize+50-15, textBase);
	            cb.saveState();   
	           // Create a School Url to the right side of the footer 
	            textSize = helv.getWidthPoint(text, 12);
	            textBase = document.bottom() - 35;
		        cb.beginText();
		        cb.setFontAndSize(helv, 7);
	            cb.setTextMatrix(document.right() - textSize-adjust+2, textBase);
	            cb.showText("Generated By : www.Eazyschool.com");
	            cb.endText();
	            cb.saveState();
	    }
	    ...
	    public void onCloseDocument(PdfWriter writer, Document document) {
	       ...
	       tpl.showText(" " + (writer.getCurrentPageNumber() - 1));
	       tpl.endText();
	    }	    
