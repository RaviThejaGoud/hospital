...
public class HSSFHelper {
	...
	public <T> T getCellValue(Sheet sheet, String sheetName, Class<T> type,
			Integer row, Integer col, boolean zeroIfNull)
			throws ExcelParsingException {
		Cell cell = getCell(sheet, row, col);
		//log.debug("****" + sheetName + "****" + row +"***** " + col ); 
		if (type.equals(String.class)) {
			return cell == null ? null : (T) getStringCell(cell);
		} else if (type.equals(Date.class)) {
			return cell == null ? null : (T) getDateCell(cell, sheetName, row,
					col);
		}else if (type.equals(long.class)) {
			return (T) getLongCell(cell, true, sheetName, row, col);
		} else if (type.equals(double.class)){
			return (T) getDoubleCell(cell, true, sheetName, row, col);
		...
		throw new ExcelParsingException(getErrorMessage(
				DATA_TYPE_NOT_SUPPORTED, type.getName()));
	...
	Date getDateCell(Cell cell, Object... errorMessageArgs)
		...
		try {
			...
			String cellValueAsString =  getStringCell(cell);
			if(StringFunctions.isNullOrEmpty(cellValueAsString))
			{
				return null;
			}
	...
	Double getDoubleCell(Cell cell, boolean zeroIfNull,
			Object... errorMessageArgs) throws ExcelParsingException {
		if (cell == null) {
		...
		default:
			throw new ExcelParsingException(getErrorMessage(
					INVALID_NUMBER_FORMAT, errorMessageArgs));
		}
	}

	Long getLongCell(Cell cell, boolean zeroIfNull, Object... errorMessageArgs)
			throws ExcelParsingException {
		Double doubleValue = getNumberWithoutDecimals(cell, zeroIfNull,
				errorMessageArgs);
		return doubleValue == null ? null : doubleValue.longValue();
	}

	Integer getIntegerCell(Cell cell, boolean zeroIfNull,
			Object... errorMessageArgs) throws ExcelParsingException {
		Double doubleValue = getNumberWithoutDecimals(cell, zeroIfNull,
				errorMessageArgs);
	...
	private Double getNumberWithoutDecimals(Cell cell, boolean zeroIfNull,
			Object... errorMessageArgs) throws ExcelParsingException {
