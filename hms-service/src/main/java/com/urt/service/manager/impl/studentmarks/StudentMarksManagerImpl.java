package com.urt.service.manager.impl.studentmarks;
...
@Component
public class StudentMarksManagerImpl extends UniversalManagerImpl implements StudentMarksManager  {
   ...
   // private List<ExamTypes> examTypes;
	...
	/*public StudentMarksManagerImpl() {
...
order by ss.subjectSortingOrder;*/
	...
	try {
	     ...
	     HSSFSheet sheet = (HSSFSheet) wb.createSheet(studyClass.getClassAndSection());
	    //sheet.createFreezePane(5, 2);
	     styles = createStyles(wb);
	    log.debug("sheet name : " + studyClass.getClassAndSection());
	    ...
	    rowNum = createSheetHeader(sheet,rowNum,examTypes);
	    ...
	    if (!ObjectFunctions.isNullOrEmpty(students)) {
			...
			for (ViewStudentClassDetails student : students) {
			    ...
			    col = 0;
			    marks = this.getAll(studentsMarksQuery+" where studentId ="+ student.getStudId()+" order by ss.subjectSortingOrder");
