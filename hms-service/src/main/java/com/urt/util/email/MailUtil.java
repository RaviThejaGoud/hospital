...
public class MailUtil {
    ...
    public MailUtil(String[] emailAddresses, String subject, String description, User user,String[] attachmentFiles,String fromEmail) {
		// this.adminManager = adminManager;
		this.emailBean = new EmailBean();
		this.emailBean.setFromAddress(fromEmail);
		//this.emailBean.setToAddress(emailAddresses);
		...
		this.emailBean.setFromWhom(user.getUserRoleDescription());
		this.emailBean.setBody(description);
		if(!ObjectFunctions.isNullOrEmpty(attachmentFiles))
    ...
    public MailUtil(String[] emailAddresses, String subject ) {
	  		this.emailBean = new EmailBean();
	  		this.emailBean.setFromAddress("admin@easyschool.in");
	  		//this.emailBean.setToAddress(emailAddresses);
	  		this.emailBean.setBccAddress(emailAddresses);
	  		this.emailBean.setSubject(subject);
	  		this.emailBean.setFromWhom("EazySchool");
    }
    ...
    public MailSenderManager getMailSender() {
		if (ObjectFunctions.isNullOrEmpty(this.mailSender)) {
		    mailSender = new MailSenderManager(this.contactEmail, this.contactPasword);
		}
    ...
    public void sendQuizAlertAndEmailToParentAnSdtudent(Student student,Person person,Date currentDate,List<QuizQuestion> questionAnswerList) {
	    try {
			String[] bodyPara = new String[4];
			StringBuffer msg = new StringBuffer();
    ...
    public void sendMailForAllParentsForFeeAmount(ViewStudentClassDetails student,SchoolTerms feeType,long dueAmount,String custOrganizationName,long totalUnpaidPreviewAmount,String fromEmail) {
	    try {
			...
			if(!StringFunctions.isNullOrEmpty(student.getParentEmail()) && !StringFunctions.isNullOrEmpty(student.getFatherName())){
				...
				content = feeType.getMailContentDesc();
				msgContent = content.replaceAll("<Children Name>",student.getFullName());
				msgContent = msgContent.replaceAll("<Term Name>",feeType.getTermName());
				msgContent = msgContent.replaceAll("<Date>",feeType.getDueDateStr());
				msgContent = msgContent.replaceAll("<School Name>",custOrganizationName);*/
    ...
    public void sendMailForAllParentsForChildrenAttendance(ViewStudentPersonAccountDetails student1,ExamSchedules examSchedules1,String totalabsentDays,String totalWorkingDays) {
    	try{
            String[] bodyPara = new String[4];
 ...
 // This method is for already existed user with another role as a parent for another student at add && edit process : on 07/07/2014 ----------rama.
    ...
    public void sendMailMontlyStudentAttendanceForParent(String studentName,String monthName,String totalWorkingDays,String totalPresentDays,String[] parentEmail,String fatherName,String classNameAndSection) {
    	try{
    ...
    public boolean sendFeeReceiptToParents(Student student,Person person, String parentEmail,List feesPaymentList, long custId, long userId)
	 ...
	 public static void SendAttachmentFilesToParentAndStudent(List<Object> studentsContactDetails, Customer customer, ClassAssignment classAssignment,String[] fileUploadStr)
	 {
		 try {
				...
				if(ObjectFunctions.isNotNullOrEmpty(studentsContactDetails)){
					for(Object stud : studentsContactDetails){
						...
						if(!ObjectFunctions.isNullOrEmpty(studentInfo[0]) && !ObjectFunctions.isNullOrEmpty(studentInfo[1]) && !ObjectFunctions.isNullOrEmpty(studentInfo[2]) 
							...
							mailUtil=new MailUtil(emailAddress,"Welcome to EazySchool Management",customer.getId(),customer.getSender(),"Administrator",customer.getContactEmail());
							mailUtil.setContactEmail(customer.getContactEmail());
							mailUtil.setContactPasword(customer.getContactPassword());
							mailUtil.sendMailForAllAttachementFilesToParentAndStudent(studentInfo,customer.getCustEmail(),customer.getOrganization(),classAssignment,fileUploadStr,customer);
							emailAddress = null;
	 ...
	 public boolean sendEmailToFeeReceiptToParent() 
	 {
		try {
    	    StringBuffer msg = new StringBuffer();
    	    msg.append("Dear Parent, ");
	 ...
	 public boolean sendEmailAndSmsForSupportUrt() {
			try {
				   ...
				   // getEmailBean().setAttachmentName("C:/Users/Public/Pictures/Desert.jpg");
				    getMailSender().sendHtmlMail(getEmailBean());
				    log.debug("Hurray success....+ email sent successfully");
				    // saveActiveLogs(getEmailBean());
					setEmailBean(null);
	 ...
	 public int sendEmailPaySlipsToStaff(String fullName,String fromEmail,String monthName) {
	    	int responseCode=0;
		try {
			    String[] bodyPara = new String[3];
			    StringBuffer msg = new StringBuffer();
