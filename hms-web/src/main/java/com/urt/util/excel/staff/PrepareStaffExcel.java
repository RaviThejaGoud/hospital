...
public class PrepareStaffExcel {
    ...
    private static final String[] titles = { "Staff Id", "Role", "First Name", "Last Name", "Initial","Date Of Birth (DD-MMM-YYYY)", "Gender",
    	...
    	"State","Postal Code","Blood Group","PAN Number","GPF No","Office Phone No","Res .phone No","Designation","Religion","Mother Tongue",
    	...
    	"Staff Number","Aadhaar Number","Passport Number","Ifsc Code","Employment Type","Staff Unique Number","Outside School Duty","School Mess","Staff Payment Mode","Staff Grade","Staff Location",
    	"Father/Spouse Name","Father/Spouse Mobile number"};

    private static final Integer[] colwidths = { 7, 15, 17, 7, 8, 15,8, 15, 15, 15, 13, 15, 25, 20, 30, 15, 15, 13,15, 15,
    	18, 18, 10, 10, 10, 15, 15, 15, 20, 35, 35, 12, 12, 12, 12, 10, 25, 10,30,15,15,10,15,10,10,25,25,25};
    ...
    public static final String query = "select staffId,roleDescription,firstName,lastName,'',dateOfBirth,gender,maritalStatus,experience,dateofJoining,qualification1,salary,email,mobileNumber,addressLine1," +
    		...
    		"staffNumber,aadharNumber,passportNumber,ifscCode,staffTypeStatus,staffUniqueNumber,outSideSchoolDuty,schoolMess,salaryPaymentMode,staffGrade,staffLocation,fatherName,fatherContactNumber from vw_staffDetails ";
    ...
    private int communitypos = 27;
    private int communityEnd = 0;
    private int castNamepos = 28;
  ...
  //  private String[] dateOfJoining = {"dd/mm/yyyy"};
    private String[] staffTypeStatus = {"Permanent","Temporary"};
    private String[] schoolMess = {"Y", "N"};
    private String[] salaryPaymentMode = {"CASH","CHEQUE","NEFT/RTGS","DD"};
    ...
    private DataValidation createMotherToungDropDownList(int srow, int erow) {
    	...
    	try {
    	    ...
    	    dataValidation = new HSSFDataValidation(addressList, dvConstraint);
    	    dataValidation.setSuppressDropDownArrow(false);
    	    dataValidation.createPromptBox("Mother Tongue", "Select Mother Tongue from dropdown!");
    	    dataValidation.setShowPromptBox(true);
    	    dataValidation.setErrorStyle(DataValidation.ErrorStyle.STOP);
    	    dataValidation.createErrorBox("Invalid Mother Tongue", "Please select the Mother Tongue from the dropdown!");
    	} catch (Exception ex) {
    	    ex.printStackTrace();
    	    JRExceptionClient jre = new JRExceptionClient();jre.sendException(ex);jre = null;
    	}
   ...
   private DataValidation createMaritalStatusDropDownList(int srow, int erow) {
    ...
    private DataValidation createSchoolMessDropDownList(int srow, int erow) {
    	DataValidation dataValidation = null;
    	try {
    	    CellRangeAddressList addressList = new CellRangeAddressList(srow, erow, 42,42);
    	    DVConstraint dvConstraint = DVConstraint.createExplicitListConstraint(schoolMess);
    	    dataValidation = new HSSFDataValidation(addressList, dvConstraint);
    	    dataValidation.setSuppressDropDownArrow(false);
    	    dataValidation.createPromptBox("schoolMess", "Select School Mess from dropdown!\n(For 'Yes' : 'Y','No' : 'N')");
    	    dataValidation.setShowPromptBox(true);
    	    dataValidation.setErrorStyle(DataValidation.ErrorStyle.STOP);
    	    dataValidation.createErrorBox("Invalid School Mess", "Please select the school Mess from the dropdown!");
    	} catch (Exception ex) {
    	    ex.printStackTrace();
    	    JRExceptionClient jre = new JRExceptionClient();jre.sendException(ex);jre = null;
    	}
    ...
    private DataValidation createStaffPaymentModeDropDownList(int srow, int erow) {
    	DataValidation dataValidation = null;
    	try {
    	    CellRangeAddressList addressList = new CellRangeAddressList(srow, erow, 43,43);
    	    DVConstraint dvConstraint = DVConstraint.createExplicitListConstraint(salaryPaymentMode);
    	    dataValidation = new HSSFDataValidation(addressList, dvConstraint);
    	    dataValidation.setSuppressDropDownArrow(false);
    	    dataValidation.createPromptBox("Salary Payment Mode", "Select Salary Payment Mode from dropdown!\n(For 'CASH' : 'CASH', 'CHEQUE' : 'CHEQUE','NEFT/RTGS' : 'NEFT/RTGS','DD' : 'DD')");
    	    dataValidation.setShowPromptBox(true);
    	    dataValidation.setErrorStyle(DataValidation.ErrorStyle.STOP);
    	    dataValidation.createErrorBox("Invalid Salary Payment Modee", "Please select the Salary Payment Mode from the dropdown!");
    	} catch (Exception ex) {
    	    ex.printStackTrace();
    	    JRExceptionClient jre = new JRExceptionClient();jre.sendException(ex);jre = null;
    	}
	...
	for (int i = 0; i < nofsheets; i++) {
	    ...
	    sheet.addValidationData(createSchoolMessDropDownList(2, lastNumber));
	    
	    sheet.addValidationData(createCommunityDropDownList(2, lastNumber));
	    sheet.addValidationData(createCasteDropDownList(2, lastNumber));
	    
	    SheetConditionalFormatting sheetCF = sheet.getSheetConditionalFormatting();
	    sheet.addValidationData(createStaffPaymentModeDropDownList(2, lastNumber));
    ...
    public void createConfigurationsSheet(String sheetName, List<State> states,List<MotherTongue> motherTongues,TreeMap<String,String> staffRoles,long academicYear, long custId,List<CastSettings> communityList,List<SubCastSettings> castNameList) {
	try {
	    //refSheet = sheetName;
	    HSSFSheet sheet = (HSSFSheet) wb.createSheet(sheetName);
	    Row row;
    ...
    private DataValidation createCommunityDropDownList(int srow, int erow) {
    	DataValidation dataValidation = null;
    	try {
    	    CellRangeAddressList addressList = new CellRangeAddressList(srow, erow, communitypos, communitypos);
    	    DVConstraint dvConstraint = DVConstraint.createFormulaListConstraint("Configurations!$H$6:$H$" + communityEnd);
    	    dataValidation = new HSSFDataValidation(addressList, dvConstraint);
    	    dataValidation.setSuppressDropDownArrow(false);
    	    dataValidation.createPromptBox("Community", "Select Community from dropdown!");
    	    dataValidation.setShowPromptBox(true);
    	    dataValidation.setErrorStyle(DataValidation.ErrorStyle.STOP);
    	    dataValidation.createErrorBox("Invalid Community", "Please select the Community from the dropdown!");
    	} catch (Exception ex) {
    	    ex.printStackTrace();
    	    JRExceptionClient jre = new JRExceptionClient();jre.sendException(ex);jre = null;
    	}
    ...
    private DataValidation createCasteDropDownList(int srow, int erow) {
    	DataValidation dataValidation = null;
    	try {
    	    CellRangeAddressList addressList = new CellRangeAddressList(srow, erow, castNamepos, castNamepos);
    	    DVConstraint dvConstraint = DVConstraint.createFormulaListConstraint("Configurations!$J$6:$J$" + castNameEnd);
    	    dataValidation = new HSSFDataValidation(addressList, dvConstraint);
    	    dataValidation.setSuppressDropDownArrow(false);
    	    dataValidation.createPromptBox("Castes", "Select Caste name from dropdown!");
    	    dataValidation.setShowPromptBox(true);
    	    dataValidation.setErrorStyle(DataValidation.ErrorStyle.STOP);
    	    dataValidation.createErrorBox("Invalid Caste", "Please select the Caste name from the dropdown!");
    	} catch (Exception ex) {
    	    ex.printStackTrace();
    	    JRExceptionClient jre = new JRExceptionClient();jre.sendException(ex);jre = null;
    	}
