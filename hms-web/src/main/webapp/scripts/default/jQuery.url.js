jQuery.url = function() {
	...
	return {
		...
		// see above
		getChatURL : function(action) {
			var hostName = key('host');
			if (hostName == 'localhost'.toLowerCase()) {
				hostName += ':8080/sms-web';
			}
			if (action)
				return key("protocol") + "://" + hostName + action;
			else
				return key("protocol") + "://" + hostName;
		}
	};
