	...
	var DataTable = function( oInit )
	{
		...
		*//**
		...
		function _fnApplyColumnDefs( oSettings, aoColDefs, aoCols, fn )
		{
			var i, iLen, j, jLen, k, kLen;
			 ...
			 Column definitions with aTargets			if ( aoColDefs )
			{
			 ...
			 Statically defined columns array			if ( aoCols )
			{
			...
			for ( iColumn=0, iColumns=oSettings.aoColumns.length ; iColumn<iColumns ; iColumn++ )
			{
				...
				if ( bAutoType || bRender || bClass || !bVisible )
				{
					for ( iRow=0, iRows=oSettings.aoData.length ; iRow<iRows ; iRow++ )
					{
						...
						if ( oCol.mRender )
						{
							 mRender has been defined, so we need to get the value and set it							nCell.innerHTML = _fnGetCellData( oSettings, iRow, iColumn, 'display' );
						}
						else if ( oCol.mData !== iColumn )
						{
							 If mData is not the same as the column number, then we need to							 get the dev set value. If it is the column, no point in wasting							 time setting the value that is already there!							nCell.innerHTML = _fnGetCellData( oSettings, iRow, iColumn, 'display' );
						}
		...
		*//**
		...
		function _fnSetCellData( oSettings, iRow, iCol, val )
		{
		 ...
		 Private variable that is used to match array syntax in the data property object		var __reArray = /\[.*?\]$/;
			...
			else if ( typeof mSource === 'string' && (mSource.indexOf('.') !== -1 || mSource.indexOf('[') !== -1) )
			{
				...
				var fetchData = function (data, type, src) {
					...
					if ( src !== "" )
					{
						...
						{
							 Check if we are dealing with an array notation request							arrayNotation = a[i].match(__reArray);
		
							if ( arrayNotation ) {
								a[i] = a[i].replace(__reArray, '');
		
								 Condition allows simply [] to be passed in								if ( a[i] !== "" ) {
									data = data[ a[i] ];
								}
								out = [];
								
								 Get the remainder of the nested object to get								a.splice( 0, i+1 );
								innerSrc = a.join('.');
		
								 Traverse each entry in the array getting the properties requested								for ( var j=0, jLen=data.length ; j<jLen ; j++ ) {
									out.push( fetchData( data[j], type, innerSrc ) );
								}
		
								 If a string is given in between the array notation indicators, that								 is used to join the strings together, otherwise an array is returned								var join = arrayNotation[0].substring(1, arrayNotation[0].length-1);
								data = (join==="") ? out : out.join(join);
		
								 The inner call to fetchData has already traversed through the remainder								 of the source requested, so we exit from the loop								break;
							}
					...
					for ( var i=0, iLen=a.length-1 ; i<iLen ; i++ )
					{
						...
						if ( arrayNotation )
						{
							...
							data[ a[i] ] = [];
							 ...
							 Get the remainder of the nested object to set so we can recurse							b = a.slice();
							b.splice( 0, i+1 );
							innerSrc = b.join('.');
		
							 Traverse each entry in the array setting the properties requested							for ( var j=0, jLen=val.length ; j<jLen ; j++ )
							{
								o = {};
							 ...
							 The inner call to setData has already traversed through the remainder							 of the source and has set the data, thus we can exit here							return;
						}
		
						 If the nested object doesn't currently exist - since we are						 trying to set the value - create it						if ( data[ a[i] ] === null || data[ a[i] ] === undefined )
						{
							data[ a[i] ] = {};
						}
						data = data[ a[i] ];
					 ...
					 If array notation is used, we just want to strip it and use the property name					 and assign the value. If it isn't used, then we get the result we want anyway					data[ a[a.length-1].replace(__reArray, '') ] = val;
				};
		...
		*//**
		...
		function _fnFeatureHtmlFilter ( oSettings )
		{
			...
			var jqFilter = $('input[type="text"]', nFilter);
			 ...
			 Store a reference to the input element, so other input elements could be			 added to the filter wrapper if needed (submit button for example)			nFilter._DT_Input = jqFilter[0];
		...
		*//**
		...
		function _fnBuildSearchRow( oSettings, aData )
		{
			...
			if ( sSearch.indexOf('&') !== -1 )
			{
			 ...
			 Strip newline characters			return sSearch.replace( /[\n\r]/g, " " );
		}
		...
		*//**
		...
		function _fnUpdateInfo ( oSettings )
		{
			...
			if ( iTotal != iMax )
			{
			 ...
			 Convert the macros			sOut += oLang.sInfoPostFix;
		...
		function _fnInfoMacros ( oSettings, str )
		{
			var
			 ...
			 When infinite scrolling, we are always starting at 1. _iDisplayStart is used only			 internally			if ( oSettings.oScroll.bInfinite )
			{
		...
		*//**
		...
		function _fnScrollDraw ( o )
		{
			...
			if ( o.nTFoot !== null )
			{
			 ...
			 If scroll collapse is enabled, when we put the headers back into the body for sizing, we			 will end up forcing the scrollbar to appear, making our measurements wrong for when we			 then hide it (end of this function), so add the header height to the body scroller.			if ( o.oScroll.bCollapse && o.oScroll.sY !== "" )
			{
			 ...
			 * set the width based on the real headers
			 ...
			 Apply all styles in one pass. Invalidates layout only once because we don't read any 			 DOM properties.			_fnApplyToChildren( zeroOut, anHeadSizers );
			 
			 Read all widths in next pass. Forces layout only once because we do not change 			 any DOM properties.			_fnApplyToChildren( function(nSizer) {
				aApplied.push( _fnStringToCss( $(nSizer).width() ) );
			}, anHeadSizers );
			 
			 Apply all widths in final pass. Invalidates layout only once because we do not			 read any DOM properties.			_fnApplyToChildren( function(nToSize, i) {
			...
			nScrollHeadInner.style.width = _fnStringToCss( iOuterWidth );
			 ...
			 Figure out if there are scrollbar present - if so then we need a the header and footer to			 provide a bit more space to allow "overflow" scrolling (i.e. past the scrollbar)			var bScrolling = $(o.nTable).height() > nScrollBody.clientHeight || $(nScrollBody).css('overflow-y') == "scroll";
		...
		function _fnCreateCookie ( sName, sValue, iSecs, sBaseName, fnCallback )
		{
			...
			if ( iNewCookieLen+document.cookie.length+10 > 4096 )  Magic 10 for padding 
			{
				for ( var i=0, iLen=aCookies.length ; i<iLen ; i++ )
				{
				 ...
				 Make sure we delete the oldest ones first				aOldCookies.sort( function (a, b) {
					return b.time - a.time;
				} );
		
				 Eliminate as many old DataTables cookies as we need to				while ( iNewCookieLen + document.cookie.length + 10 > 4096 ) {
					if ( aOldCookies.length === 0 ) {
						 Deleted all DT cookies and still not enough space. Can't state save						return;
					}
		...
		function _fnBindAction( n, oData, fn )
		{
			$(n)
				.bind( 'click.DT', oData, function (e) {
						n.blur();  Remove focus outline for mouse users						fn(e);
					} )
		...
		*//**
		...
		var _fnJsonString = (window.JSON) ? JSON.stringify : function( o )
		{
			...
			{
				 simple data type				if (sType === "string")
				{
					o = '"'+o+'"';
				}
				return o+"";
		...
		*//**
		...
		this.$ = function ( sSelector, oOpts )
		{
			...
			oOpts = $.extend( {}, {
				"filter": "none",  applied				"order": "current",  "original"				"page": "all"  current			}, oOpts );
		
			 Current page implies that order=current and fitler=applied, since it is fairly			 senseless otherwise			if ( oOpts.page == 'current' )
			{
		...
		this.fnFilter = function( sInput, iColumn, bRegex, bSmart, bShowGlobal, bCaseInsensitive )
		{
			...
			if ( iColumn === undefined || iColumn === null )
			{
				...
				if ( bShowGlobal && oSettings.aanFeatures.f )
				{
					...
					for ( var i=0, iLen=n.length ; i<iLen ; i++ )
					{
						 IE9 throws an 'unknown error' if document.activeElement is used						 inside an iframe or frame...						try {
		...
		*//**
		...
		this.each(function() {
			...
			DataTable.settings.push( oSettings );
			 ...
			 Need to add the instance after the instance after the settings object has been added			 to the settings array, so we can self reference the table instance if more than one			oSettings.oInstance = (_that.length===1) ? _that : $(this).dataTable();
			...
			if ( !oInit )
			{
			 ...
			 Backwards compatibility, before we apply all the defaults			if ( oInit.oLanguage )
			{
			...
			oInit = _fnExtend( $.extend(true, {}, DataTable.defaults), oInit );
			 ...
			 Map the initialisation options onto the settings object			_fnMap( oSettings.oFeatures, oInit, "bPaginate" );
			...
			_fnMap( oSettings, oInit, "asStripClasses", "asStripeClasses" );  legacy			_fnMap( oSettings, oInit, "fnServerData" );
			...
			_fnBrowserDetect( oSettings );
			 ...
			 Work around for Webkit bug 83867 - store the caption-side before removing from doc			var captions = $(this).children('caption').each( function () {
			...
			{
				 If we are a scrolling table, and no footer has been given, then we need to create				 a tfoot element for the caption element to be appended to				tfoot = [ document.createElement( 'tfoot' ) ];
	...
	DataTable.defaults = {
		...
		*//**
		...
		"fnFormatNumber": function ( iIn ) {
			if ( iIn < 1000 )
			{
				 A small optimisation for what is likely to be the majority of use cases				return iIn;
			}
