...
changePageTitle('Student Events');
...
/*
					...
					cal2.eventDates = new Array(
						  new Array("2010/10/10", 1)
						, new Array("2010/10/12", 2)
						, new Array("2010/10/14", 3)
					);
	...
	*/
	...
	$(document).ready(function() {		
	    $.ajax( {
			url : jQuery.url.getChatURL("/student/ajaxGetEventDates.do"),
			cache : false,
			dataType : 'json',
			success : function(response) {
				var studentEvents=[];
				var eventDates=[];
				studentEvents = response.eventDatesSet;	
				cal2 = new Calendar ("cal2", "tester2", new Date());
				cal2.scrolling = false;
				if (studentEvents.length >= 1) {
				    var eventlength=studentEvents.length;
				    var eventDateArray=new Array();
					for ( var i = 0; i < eventlength; i++) {
					   var pdate = new Date(studentEvents[i]);
					   var stringdate=pdate.getFullYear()+"/"+(pdate.getMonth()+1)+"/"+pdate.getDate();
					   eventDateArray[i]= new Array(stringdate,stringdate);
					   //new Array(pdate.getFullYear()+"/"+(pdate.getMonth()+1)+"/"+pdate.getDate(), studentEvents[i].id);	
					}				
					cal2.eventDates = eventDateArray;
					/*cal2.eventDates = new Array(
						  new Array("2010/10/04", 1)
						, new Array("2010/10/12", 2)
						, new Array("2010/10/14", 3)
					);*/					
					cal2.selectEvent = function(eventDate) {
						 $.ajax( {
							url : jQuery.url.getChatURL("/student/ajaxGetEventDetails.do?eventDate="+eventDate),
							cache : false,
							success : function(response) {
							     $('#studentContent').html(response);
							}
						});
						//alert("cal2 Event: " + eventId);
					};							
				 }
				 renderCalendar (cal2);		
			}
