...
<s:if test="%{messages != null}">
	<s:form action="ajaxReplayMessageToStaff" theme="css_xhtml"
		id="formApprovalLeaves" namespace="/student">
		<s:hidden name="messages.senderAccountId" />
		<s:hidden name="messages.receiverAccountId" />
		<s:hidden name="anyId" />
		<div class="grid_6">
		...
		<div class="grid_6">
			<div class="grid_4" style="text-align: left;">
				<sj:submit   cssClass="submit small" value="Submit"
					indicator="indicator" targets="myMessagesContent"
					onClickTopics="formValidationForApprovalLeaves" />

				<s:url id="doCancelReplyMessage" action="ajaxDoGetTeacherMessagesForParent"
					includeParams="all" namespace="/student"></s:url>
				<sj:a href="%{doCancelReplyMessage}" cssClass="cancelButton"
					indicator="indicator" targets="myMessagesContent">Cancel</sj:a>
			</div>
		</div>
	</s:form>
	</s:if>
