...
<div id="searchStudentsForm112" class="commonFormTabs">
	<!--<div class="grid_15">
		...
		<div class="grid_10" align="left">
					<label >Select Fee Setting Type:</label>
					<s:select list="objectList" listKey="id" listValue="settingName"
						required="true" id="settingFee" name="tempId" theme="simple"
						headerKey="null" label="Select Fee Setting Type" onchange="javascript:getAjaxGetFeeSettingParticular(this.value);"
						headerValue="-Select Fee Setting Type-" requiredposition="first">
					</s:select>
				</div>
	</div>
	--><div id="tabWrapper">
		<s:form action="ajaxPayStudentPaymentFee" id="addStudentPaymentFee" method="post" theme="css_xhtml" name="myform">
			...
			<fieldset>
				<div class="grid_15">
					...
					<div class="grid_15">
						...
						<div class="grid_8">
							<sj:textfield name="studentPayment.invoiceNumber" id="studReceiptNumber"
								cssClass="numeric textfield" maxlength="11"></sj:textfield>
								<span class="hintMessage">(Please provide the receipt numbers in the above text box. Otherwise system will generate and assign the receipt number.)</span>
						</div>
						<div class="grid_2">
							&nbsp;
						</div>					
					...
					<div class="grid_15">
						...
						<div class="grid_8">
							<sj:datepicker id="paymentDate" name="studentPayment.paymentDate" 
						readonly="true" changeMonth="true" changeYear="true" cssClass="textfield" maxDate="0"/>
						</div>
						<div class="grid_2">
							&nbsp;
						</div>					
					...
					<div class="grid_15">
						...
						<div class="grid_5">
							<sj:textfield name="discountAmount" id="termDescountAmount"
								cssClass="numeric textfield defaultValue"
					...
					<div class="grid_15">
						...
						<div class="grid_5">
							<sj:textfield name="totalAmount"
								id="totalPayAmountAfterDiscount" cssClass="numeric textfield defaultValue"
					...
					<div class="grid_15">
						...
						<input type="radio" value="C" name="payType"
								onclick="javascript:paymentTypeMethodChange(this.value)" checked>
							Cash&nbsp;&nbsp;
							<input type="radio" value="D" name="payType" id="ddForm"
								onclick="javascript:paymentTypeMethodChange(this.value)">
							DD&nbsp;&nbsp;
							<input type="radio" value="CH" name="payType" id="chForm"
								onclick="javascript:paymentTypeMethodChange(this.value)">Cheque
					</div>
					<div class="grid_15">&nbsp;</div>
			...
			<div id="steps">
				<s:iterator value="classFeeList">
									...
									<s:iterator value="schoolFeeList">
									 <s:if test='%{invoiceNumber > 0}'>
										<a id="noPrint"
                                            ...
                                            target="_new"> <img style="" src="../images/index.jpg"> Invoice : <s:property value="invoiceNumber" /> </a>
										
										<%--<a href="${pageContext.request.contextPath}/reports/ajaxStudentPaymentPdfFeeReportPopUp.do?spId=
											<s:property value="studentId"/>&createdDate=<s:property value="ctrDateStr"/>
											&invoiceNumber=<s:property value="invoiceNumber"/>"
											class="invoiceStudent tabClass" title="View Events"><img style="" src="../images/index.jpg">
										<s:property value="createdDateStr" /> </a>
									 --%>
									 
									 </s:if>
									<!--<iframe id="iFramePdf" src="<c:url value='/admin/ajaxStudentPaymentPdfReport.do?pdfId=pdf'/>&spId=<s:property value="studentId"/>&createdDate=<s:property value="createdDateStr"/>" style="dispaly:none;"></iframe>
									-->
					...
					<div class="grid_3">
						<s:if test='%{totalDiscountAmount > 0 || discountAmount == null}'>
							<del>
								<b style="float: left;">&#2352;</b>
							</del>
...
function onChangeTotal(serviceId) {
	...
	if(parseFloat(totalAmount) <= parseFloat(serviceId)){
		...
		alert("Total amount should be less than due amount.");
	}else if(serviceId==0){
	  alert("Total fee should be some amount not 0 value.");
	  return false;
	}
}
$(document).ready(function() {
	commonLoadTab();
	$("#studReceiptNumber").autoCheck("${pageContext.request.contextPath}/admin/ajaxCheckStudentReceiptNumber.do",{
										minChars : 1,
										min : "no"
									});
	$.subscribe('studentPaymentFormValidation', function(event, data) {
		...
		if ($('#addStudentPaymentFee').valid()) {
			...
			if (isNonEmpty(receiptNumber)){
				...
				var r=confirm("Are you sure system want to generate the receipt number?");
				 	if(r==true){
				 		return true;
				 	}else{
				 	return false;
...
function getTermsTotalAmount(status) {
	var levelId = document.getElementsByName("chkBoxFeeSelectedIds");
	var amount = 0;
		for (i = 0; i < levelId.length; i++) {
			if (levelId[i].checked) {
...
function checkCurrentForm(id) {
		...
		if ($('#tabNavigation ul li a  div').hasClass("ribbon").toString()!="false") {
