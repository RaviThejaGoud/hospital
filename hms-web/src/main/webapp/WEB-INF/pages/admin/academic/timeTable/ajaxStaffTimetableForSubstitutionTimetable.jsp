...
<s:hidden name="anyId" id="weekDayId"></s:hidden>
...
<div id="divId"> </div>
	<table
		...
		<sj:submit value="Submit" id="submitButtonMainContent"
								targets="viewTimetableContDiv" indicator="indicator"
								cssClass="submitBt btn blue" formIds="substitutionTimetableFormId"
								onBeforeTopics="staffSubstitutionTimetableTopics" /> 
								
								
			</s:if>
			<s:else>
				<div class="alert alert-info">
					Currently there are no study class.
				</div>
			</s:else>
...
$(document).ready(function() {
	...
	var staffIdsId = $('#staffIdsId').val();
	var weekDayId = $('#weekDayId').val();
	var dataURL = jQuery.url.getChatURL("/timeTable/ajaxGetStaffWiseTimetableForSubstitution.do?tempId=" + staffId +"&anyTitle="+staffIdsId +"&anyId="+weekDayId +"&attendanceDate="+attendanceDate);
	$.ajax( {
				url : dataURL,
				cache : false,
				dataType : 'json',
...
$.subscribe('staffSubstitutionTimetableTopics', function(event, data) {
	if ($('#substitutionTimetableFormId').valid()){
			return true;
	}else{
