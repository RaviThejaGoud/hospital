...
<s:form id="studentGenerateTcInfoForm" method="post" theme="simple"
	...
	<s:hidden name="empId" cssClass="studentsId" id="studentIds"></s:hidden>
		...
		<div class="panel-body">
			...
			<div class="col-md-9">
				<ul>
					...
					<li>
						Before generating tc make sure tc template is uploaded for all
						classes or not. In TC settings
					</li>
				</ul>
			</div>
		</div>
			...
			<thead class="flip-content">
				<tr>
					...
					<s:if test='%{#session.previousYear=="N"}'>
						<th>
							 Inactive <input type="checkbox" id="selectall" >
						</th>
					</s:if>
				</tr>
			</thead>
				<s:iterator value="studentsList" status="stat">
						<tr class="feeNotPadiStudsDate">
							...
							<s:if test='%{studentsList[#stat.count-1][9] !=null}'>
								<s:if test='%{studentsList[#stat.count-1][5] == "P"}'>
									<input type="checkbox" id="disaStud_<s:property value='studentsList[#stat.count-1][6]'/>" name="chkBoxInActiveIds" value="<s:property value='studentsList[#stat.count-1][6]'/>"  disabled="disabled"/>
								</s:if>
								<s:else>
									<input type="checkbox" id="disaStud_<s:property value='studentsList[#stat.count-1][6]'/>" name="chkBoxInActiveIds" value="<s:property value='studentsList[#stat.count-1][6]'/>" disabled="disabled"/>
								</s:else>
							...
							<s:else>
								<s:if test='%{studentsList[#stat.count-1][5] == "P"}'>
									<input type="checkbox" id="disaStud_<s:property value='studentsList[#stat.count-1][6]'/>" name="chkBoxInActiveIds" value="<s:property value='studentsList[#stat.count-1][6]'/>" class="checkbox anySearchCheckedId"/>
								</s:if>
								<s:else>
									<input type="checkbox" id="disaStud_<s:property value='studentsList[#stat.count-1][6]'/>" name="chkBoxInActiveIds" value="<s:property value='studentsList[#stat.count-1][6]'/>" class="checkbox anySearchCheckedId" />
								</s:else>
