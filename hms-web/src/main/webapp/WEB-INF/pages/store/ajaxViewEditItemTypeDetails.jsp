	...
	<s:form id="storeitemDetailform" cssClass="form-horizontal"
		...
		<s:if test="%{itemTypesList != null && !itemTypesList.isEmpty()}">
			<p class="text-primary">
				<b>Total Available Item Types : <s:property
						value="itemTypesList.size" /></b>
			</p>
			<table
				...
				<tbody>
					<s:iterator value="itemTypesList">
						<tr>
							...
							<td><s:property value="measurementType" /></td>
							<s:if test='%{user.isStoreKeeper !="Y" }'>
								<td><a data-toggle="modal" href="#responsive2"
									class="btn btn-xs purple"
									onclick="javascript:PopupUpdateItemTypeInfo(<s:property value="%{id}"/>)"><i
										class="fa fa-edit"></i>Edit </a></td>
								<td><s:url id="removeItemType" action="ajaxRemoveItem"
		...
		<s:else>
			<div class="alert alert-info">Currently there are no item types
				available.</div>
		</s:else>
	</s:form>
</div>
