...
<div style="padding: 20px;">
	...
	<s:if test='%{messages.isParent != "Y" }'>
		<s:url id="doCancelGroup" action="ajaxDoGetReadTeacherMessages"
			includeParams="all"></s:url>
		<sj:a href="%{doCancelGroup}" indicator="indicator"
			targets="myMessageContent" button="false">Back</sj:a>
	</s:if>
	<s:else>
		<s:url id="doCancelGroup" action="ajaxDoGetParentMessages"
			includeParams="all"></s:url>
		<sj:a href="%{doCancelGroup}" indicator="indicator"
			targets="myMessageContent" button="false">Back</sj:a>
	</s:else>
