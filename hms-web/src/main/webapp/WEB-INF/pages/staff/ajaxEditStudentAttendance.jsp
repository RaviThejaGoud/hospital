				  ...
				  <table class="striped" width="100%" cellpadding="1" cellspacing="1" id="results">
					...
					<s:iterator value="attendanceList">
						<tr class="loaded" > 
							...
							<td>
							 <s:if test='%{anyString == "A"}'>
								<input type="checkbox" name="chkBoxSelectedIds" value="<s:property value="id" />"/> 
							 </s:if>
							 <s:elseif test='%{anyString == "L"}'>
								<input type="checkbox" name="chkBoxSelectedIds" value="<s:property value="id" />" disabled="disabled" />(This student Already Approved the leave)
							</s:elseif>
							<s:else>
...
for (i = 0; i < field.length; i++)
   ...
   $.subscribe('addFormValidation', function(event, data) {
         if ($('#addSubscribedUsers').valid())
         	 return true;
          else
          	 return false;
   });
