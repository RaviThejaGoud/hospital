	...
	<div class="col-md-12">
		<div class="portlet box blue">
			<div class="portlet-title">
				<div class="caption">
					<i class="fa fa-globe"></i>Customer Register
				</div>
			</div>
			<div class="portlet-body" id="customerDetails">
				<s:if test='%{user.isDEO != "Y" && user.isBEO != "Y" && user.isSEO != "Y" && user.isCEO != "Y"}'>
					<div class="col-md-12">
						<div class="radio-list">
							<label class="radio-inline">
								<input type="radio" name="accountType" value="C"
									id="customerCreation"
									onclick="getAjaxDoGetCustomer(this.value);" checked>
								Customers
							</label>
							<label class="radio-inline">
								<input type="radio" name="accountType" value="D"
									id="govCustCreation"
									onclick="getAjaxDoGetCustomer(this.value);" >
								Demo Account
							</label>
						</div>
					</div>
...
function getAjaxDoGetCustomer(cId) {
	$("#showHideGovtCustomers")
			.html(
					'<div align="center" style="margin: 150px;"><img src="../images/icons/bigWaiting.gif" style="background-repeat: no-repeat;" border="none" title="Processing..." alt="Processing..." align="absmiddle">&nbsp;Loading ...</div>');
	var url = jQuery.url.getChatURL("/signup/ajaxMasterCustomerRegistation.do");
	$.ajax( {
		url : url,
		cache : false,
		data : "custType=" + cId,
		success : function(html) {
			if (cId == 'GC') {
				$("#govtCustomerDetails").html(html);
				$("#showHideGovtCustomers").hide();
				$("#govtCustomerDetails").show();
			} else {
				$("#showHideGovtCustomers").html(html);
				$("#showHideGovtCustomers").show();
				$("#govtCustomerDetails").hide();
			}
		}
	});
}
